...
class MediaApi
{
     ...
     * Get media by id
     *
     * @param  string $uid Alphanumeric ID of the media to get. (required)
     *
    ...
    {
        list($response) = $this->getMediaWithHttpInfo($uid);
        return $response;
    }
     ...
     */
    public function getMediaWithHttpInfo($uid)
    {
        $returnType = '\SplFileObject';
        $request = $this->getMediaRequest($uid);
     ...
     */
    public function getMediaAsync($uid)
    {
        return $this->getMediaAsyncWithHttpInfo($uid)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getMediaAsyncWithHttpInfo($uid)
    {
        $returnType = '\SplFileObject';
        $request = $this->getMediaRequest($uid);
     ...
     */
    protected function getMediaRequest($uid)
    {
        // verify the required parameter 'uid' is set
        if ($uid === null || (is_array($uid) && count($uid) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $uid when calling getMedia'
            );
        }

        $resourcePath = '/media/{uid}/';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
